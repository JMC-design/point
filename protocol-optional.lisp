...
(defgeneric / (p1 thing) ;judicious allowance to divide by zero, solution for all number types? When you make up things what do you allow?
  ...
  (:method ((p1 integer) (p2 complex)) (complex (cl:/ p2 p1)))
  ;;scalars
  (:method ((p1 3dpoint) (scalar number)) (with-dot (3d (/ p1.x scalar) (/ p1.y scalar) (/ p1.z scalar))))
  (:method ((p1 2dpoint) (scalar number)) (with-dot (2d (/ p1.x scalar) (/ p1.y scalar))))
  (:method ((p1 vector)  (scalar number)) (map 'vector (lambda (x) (cl:/ x scalar)) p1))
  (:method ((p1 cons)    (scalar number)) (cons (cl:/ (car p1) scalar) (cl:/ (cdr p1) scalar)))
  (:method ((p1 complex) (scalar number)) (complex (cl:/ (realpart p1)scalar) (cl:/ (imagpart p1) scalar)))
  (:method ((p1 number)  (scalar number)) (complex (cl:/ (realpart p1)scalar) 0)) ;assume numbers are complex numbers with a 0 imaginary part.  What could go wrong?
  ;;hopefully combinations of valid points
  (:method (p1 p2)(with-dot
                    (let ((p (copy p1)))
                      (setf p.x (/ p1.x p2.x)
